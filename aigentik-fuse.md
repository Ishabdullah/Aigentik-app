...
### Step 1: SMS/RCS inline reply (unblocks all agent action) ✅ COMPLETE 2026-03-12

1. ✅ Remove `SMSReceiver` from `AndroidManifest.xml` and remove `SEND_SMS`, `RECEIVE_SMS`, `READ_SMS`
2. ✅ Add `WAKE_LOCK`, `RECEIVE_BOOT_COMPLETED`, `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`, `READ_CONTACTS`, `FOREGROUND_SERVICE_DATA_SYNC`; fix `AigentikNotificationListener` `android:exported="true"`
3. ✅ Replaced `AigentikNotificationListener` body with thin KoinComponent adapter pattern (data extraction only, routing in injectable classes)
4. ✅ Ported `NotificationReplyRouter` (ConcurrentHashMap dual-key, RemoteInput inline reply, self-reply loop guard) and `MessageDeduplicator` (body-only fingerprint, TTL-based)
5. ✅ Wired `AigentikNotificationListener` → `AgentMessageEngine.onMessageReceived()` stub (logs receipt, no LLM yet)
6. ⬜ Test: install APK → grant notification access → send SMS → confirm logcat shows `AgentMessageEngine: onMessageReceived` → confirm inline reply capability registered
