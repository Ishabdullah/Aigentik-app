...
// ContactEngine v0.6 — ported verbatim from aigentik-android
// Thread-safe in-memory cache + Room persistence for contact intelligence.
...
object ContactEngine {
    ...
    fun formatContact(contact: Contact): String {
        ...
        contact.name?.let { lines.add("Name: $it") }
        contact.relationship?.let { lines.add("Relationship: $it") }
        if (contact.phones.isNotEmpty()) lines.add("Phone: ${contact.phones.first()}")
        if (contact.emails.isNotEmpty()) lines.add("Email: ${contact.emails.first()}")
        contact.notes?.let { lines.add("Notes: $it") }
        contact.instructions?.let { lines.add("Instructions: $it") }
